...
describe('parseCheckoutBody', () => {
  ...
  // docs/LOCKED.md invariant 1: buying must never depend on having a phone.
  // Our customers are abroad with roaming off. A `min(6)` on this field would
  // 400 exactly the travellers the invariant exists to protect, so these two
  // cases are a lock, not a preference.
  it('accepts a customer with no phone at all', () => {
    const { customer } = parseCheckoutBody({ customer: { ...validCustomer, phone: '' } });
    expect(customer.phone).toBe('');
  });

  it('accepts a customer with the phone field omitted entirely', () => {
    const { phone, ...noPhone } = validCustomer;
    expect(() => parseCheckoutBody({ customer: noPhone })).not.toThrow();
  });

  it('still rejects a phone number that was typed but is too short', () => {
    expect(() => parseCheckoutBody({ customer: { ...validCustomer, phone: '123' } })).toThrow(
      ApiError
    );
  });

  it('defaults the delivery channel to email when the client sends none', () => {
    const { customer } = parseCheckoutBody({ customer: validCustomer });
    expect(customer.deliveryChannel).toBe('email');
  });
