...
CREATE TRIGGER itinerary_places_touch
  BEFORE UPDATE ON itinerary_places
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();
...
CREATE POLICY "itinerary_places_owner" ON itinerary_places
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM itinerary_days d
      JOIN trip_plans t ON t.id = d.trip_id
