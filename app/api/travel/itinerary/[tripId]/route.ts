...
export const PATCH = route(async (request, context) => {
  ...
  if (body.data.action === 'addIdea') {
    // Same insert, now shared with POST /api/travel/saved-places so a place kept
    // from a destination guide lands in exactly the same Ideas list.
    await addIdeaToTrip(supabase, {
      tripId,
