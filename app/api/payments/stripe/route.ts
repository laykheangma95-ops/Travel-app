// POST /api/payments/stripe — price the cart, create the order, start payment
...
export const POST = route(
  async (request) => {
    const started = await startPayment(request, 'stripe');
...
export const PUT = route(
  async (request) => ok(await handlePaymentWebhook(request, 'stripe')),
